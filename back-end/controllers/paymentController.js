...
exports.initializePayment = async (req, res) => {
  try {
    const { planTitle, planPrice, currency, paymentMethod, gender, productId, totalPasses, packageId } = req.body;
    const userId = req.user.id;

    let resolvedPlanTitle = planTitle;
    let resolvedPlanPrice = planPrice;
    let resolvedTotalPasses = totalPasses;
    let resolvedDuration = null;

    // If packageId is provided, fetch package details
    let pkg = null;
    if (packageId) {
      pkg = await db.Package.findByPk(packageId);
      if (!pkg) {
        return res.status(400).json({ message: 'Package not found' });
      }
      resolvedPlanTitle = pkg.name;
      resolvedPlanPrice = pkg.price;
      resolvedTotalPasses = pkg.numberOfPasses || 0;
      resolvedDuration = pkg.duration;
    }

    if (!resolvedPlanTitle || !resolvedPlanPrice) {
    ...
    if (resolvedDuration) {
      expiryDate = new Date();
      expiryDate.setDate(expiryDate.getDate() + resolvedDuration);
    }
    ...
    if (paymentMethod === 'incash') {
      ...
      if (existingPayment) {
        existingPayment.planTitle = resolvedPlanTitle;
        existingPayment.amount = resolvedPlanPrice;
        existingPayment.gender = gender;
        existingPayment.currency = currency;
        existingPayment.txRef = txRef;
        existingPayment.productId = productId || null;
        existingPayment.totalPasses = resolvedTotalPasses || 0;
        if (expiryDate) existingPayment.expiryDate = expiryDate;
        ...
        payment = await Payment.create({
          ...
          totalPasses: resolvedTotalPasses || 0,
